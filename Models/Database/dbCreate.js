function create(con)
{
  ...
  database.database.query(query1,function(error,results){
    if(error)
    {
      return console.error(error.message);
    }
    database.database.query(query2,function(error,results){
      if(error)
      {
        return console.error(error.message);
      }
      database.database.query(query3,function(error,results){
        if(error)
        {
          return console.error(error.message);
        }
